...
// Owner-uploaded photos have no way to know what they show, so we can only
// give a generic, position-based alt (unlike the seed data, which knows
// exterior/interior/bedroom/kitchen ahead of time — see db/seed.mjs).
function photoAlt(title, index) {
  return index === 0 ? `${title} — cover photo` : `${title} — photo ${index + 1}`;
}
...
const CARD_FIELDS = `
  ...
  (select alt from property_images pi where pi.property_id = p.id order by pi.is_cover desc, pi.sort_order asc limit 1) as cover_image_alt,
...
export async function getPropertyBySlug(slug) {
  ...
  const [{ rows: images }, { rows: facilities }] = await Promise.all([
    query(
      `select id, url, alt, is_cover, sort_order from property_images where property_id = $1 order by sort_order asc`,
      [property.id]
    ),
    query(
  ...
  return withTransaction(async (client) => {
    ...
    if (Array.isArray(data.image_urls) && data.image_urls.length) {
      for (let i = 0; i < data.image_urls.length; i++) {
        await client.query(
          `insert into property_images (property_id, url, alt, is_cover, sort_order) values ($1,$2,$3,$4,$5)`,
          [property.id, data.image_urls[i], photoAlt(property.title, i), i === 0, i]
        );
      }
    }
  ...
  return withTransaction(async (client) => {
    ...
    if (Array.isArray(data.image_urls)) {
      ...
      if (data.image_urls.length) {
        const titleForAlt = data.title || (await client.query(`select title from properties where id = $1`, [id])).rows[0]?.title || "this property";
        for (let i = 0; i < data.image_urls.length; i++) {
          await client.query(
            `insert into property_images (property_id, url, alt, is_cover, sort_order) values ($1,$2,$3,$4,$5)`,
            [id, data.image_urls[i], photoAlt(titleForAlt, i), i === 0, i]
          );
        }
      }
    }
