...
export default async function ComparePage() {
  ...
  const rows = [
    ...
    { label: "Location", render: (p) => p.location_name },
    {
      ...
      render: (p) => formatDistance(p.distance_to_campus_km) || "—",
    },
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full min-w-[640px] border-collapse text-sm">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 w-40 bg-surface p-3 text-left align-bottom text-xs font-medium text-muted">
                {" "}
              </th>
              {properties.map((p) => (
                <th
                  key={p.id}
                  className="min-w-[200px] border-l border-border bg-surface p-3 text-left align-top"
                >
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} className="border-t border-border">
                <th className="sticky left-0 z-10 bg-paper p-3 text-left text-xs font-medium text-muted">
                  {row.label}
                </th>
                {properties.map((p) => (
                  <td
                    key={p.id}
                    className="border-l border-border p-3 text-ink"
                  >
                    {row.render(p)}
                  </td>
                ))}
