...
export default async function StudentConversationPage({ params }) {
  ...
  return (
    <DashboardShell variant="student" title="Messages">
      <div className="grid grid-cols-1 overflow-hidden rounded-xl border border-border lg:grid-cols-[280px_1fr]">
        <div className="hidden border-r border-border lg:block">
          <ConversationList conversations={conversations} role="student" activeId={id} />
        </div>
        <div>
          <div className="flex items-center gap-2 border-b border-border p-3 lg:hidden">
            <Link href="/dashboard/messages" className="rounded-full p-1.5 hover:bg-surface">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <div>
              <p className="text-sm font-medium text-ink">{conversation.owner_name}</p>
